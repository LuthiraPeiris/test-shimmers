  ...
  try {
    ...
    await pool.execute(
      `INSERT INTO quatation_data 
       (Quatation_Id, Customer_Name, Date_Created, Valid_Until, Status,Item_Code,Item_Name,Quantity,Total_Amount, Discount, Grand_Total)
       VALUES (?, ?, ?, ?, ?, ?, ?,?,?, ?, ?)`,
      [
        newId,
        Customer_Name,
  ...
  try {
    ...
    await pool.execute(
      `UPDATE quatation_data SET
        Customer_Name = ?, Date_Created = ?, Valid_Until = ?, Status = ?,Item_Code=?,Item_Name=?,Quantity=?, Total_Amount = ?, Discount = ?, Grand_Total = ?
        WHERE Quatation_Id = ?`,
      [
        ...
        body.Item_Code || quotation.Item_Code,
        body.Item_Name || quotation.Item_Name,
        body.Quantity || quotation.Quantity,
